...
The `CustomGlobe` component is the heart of the application. It initializes the MapLibre map and exposes it as a context to its children. It handles the rendering of the map and the layers.
...
The `Builder` is the administrative interface used to configure the map state. It wraps the `CustomGlobe` and provides a UI to manipulate the state that is passed down to the `CustomGlobe`.
...
The application state is primarily serializable to the URL hash, allowing configurations to be shared and restored.
