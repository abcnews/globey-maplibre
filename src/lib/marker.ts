 ...
 */
export async function encodeFragment({ coords = [0, 0], z, labels, layers, atmosphere, customPoints, legend, ...extraProps } = {}) {
 ...
 * ```
 */
export async function decodeObject(props, isLive = false) {
  if (!props) return {};
  const { z, geohash, labels: encodedLabels = [], custompoints: encodedLegacyCustomPoints, cstompoints: encodedCustomPoints, layers, atmosphere, legend, ...extraProps } = props;
  ...
  const labels = normalisedEncodedLabels?.map(string => {
  ...
  const customPoints = decodedCustomPoints?.map(([name, geohash, colour]) => {
  ...
  const decodedObject = {
    z: Number(z) || 200,
    coords: [decodedGeo.lon, decodedGeo.lat].map(Number),
    labels,
