<script lang="ts">
   ...
   * If jsonBlob exists, render Builder.layers/Builder.markers depending on the active mode.
   * Otherwise, render Builder.firstrun to prompt user.
   */
  const hasBlob = $derived($jsonBlob !== null);

  type Mode = 'layout' | 'markers';
   ...
   *  independently-persisted mode flag to fall out of sync with it. */
  function modeFromHash(hash: string): Mode {
    return hash.replace(/^#/, '').startsWith('mark') ? 'markers' : 'layout';
  }
