...
{#snippet Sidebar()}
	...
	{#if map && options}
		...
		/>
		<PropGeoJSON
			geoJsonList={options.geoJson || []}
			onchange={(list) => (options.geoJson = list)}
		/>
		<PropImageSource
			imageSources={options.imageSources || []}
			onchange={(list) => (options.imageSources = list)}
		/>
		<MarkerAdmin />
		<fieldset>
			<legend>Tools</legend>
			<div
				style="margin-bottom: 0.5rem; display: flex; align-items: center; gap: 0.5rem; font-size: 0.8rem;"
			>
				<input
