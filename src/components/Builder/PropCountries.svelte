<script lang="ts">
  ...
  let selectedCountries = $state<Country[]>([]);
  ...
  function setCountryStyle(iso_a2: string, style: 'primary' | 'secondary' | null) {
    const existingIndex = selectedCountries.findIndex(c => c.code === iso_a2);
  ...
  function getCountryStyle(iso_a2?: string): 'primary' | 'secondary' | null {
    ...
    const country = selectedCountries.find(c => c.code === iso_a2);
