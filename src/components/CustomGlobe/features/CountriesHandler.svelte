<script lang="ts">
  ...
  let primaryCountries = $derived(countries.filter(c => c.style === 'primary').map(c => c.code));
  let secondaryCountries = $derived(countries.filter(c => c.style === 'secondary').map(c => c.code));
